...
fn solve(input: &mut Input, out: &mut Output, _test_case: usize, precalc: &mut PreCalc) {
    ...
    for i in 0..=n {
        let mut choose = 1;
        let mut choose_suffix_sum = 1;
        let mut prev_dp_suffix_sum = 0;
        let mut prev_ways_suffix_sum = 0;
        for k in c[i] + 1..=prev_cnt {
            prev_ways_suffix_sum = add(prev_ways_suffix_sum, ways[k]);
            prev_dp_suffix_sum = add(prev_dp_suffix_sum, dp[k]);
            ways[k] = 0;
            dp[k] = 0;
        }
        for k in (0..=c[i]).rev() {
            ...
            ways[k] = add(
                mult(choose, prev_ways_suffix_sum),
                mult(choose_suffix_sum, ways[k]),
            );
            // dp[i][k] = k * ways[i][k] + choose * prev_dp_suffix_sum + choose_suffix_sum * dp[i-1][k])
            dp[k] = add(
                mult(k as i32, ways[k]),
                add(
                    mult(choose, prev_dp_suffix_sum),
                    mult(choose_suffix_sum, dp[k]),
                ),
            );
            prev_ways_suffix_sum = add(prev_ways_suffix_sum, prev_ways);
            prev_dp_suffix_sum = add(prev_dp_suffix_sum, prev_dp);
            choose = mult(choose, k as i32);
            choose = mult(choose, precalc.inv[c[i] - k + 1]);
            choose_suffix_sum = add(choose_suffix_sum, choose);
        }
        prev_cnt = c[i];
    }
    out.print_line(dp[0]);
