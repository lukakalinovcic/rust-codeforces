...
fn doit(lr: Vec<(i32, i32)>, precalc: &PreCalc) -> i32 {
    ...
    for i in 1..=n {
        while j > 0 && !(asc_order[i].r < desc_order[j].l) {
            ...
            sum_choose = mult(
                add(sum_choose, choose(precalc, j + i - 1, i - 1)),
                precalc.inv[2],
            );
        }
        let ways = sub(pow2[j + i - 1], sum_choose);
        result = sub(result, mult(mult(asc_order[i].r, ways), pow2[n - i - j]));

        sum_choose = add(mult(2, sum_choose), choose(precalc, j + i - 1, i));
    }
    ...
    for j in 1..=n {
        while i > 0 && !(asc_order[i].r < desc_order[j].l) {
            ...
            sum_choose = mult(
                add(sum_choose, choose(precalc, j + i - 1, j - 1)),
                precalc.inv[2],
            );
        }
        let ways = sub(pow2[j + i - 1], sum_choose);
        result = add(result, mult(mult(desc_order[j].l, ways), pow2[n - i - j]));

        sum_choose = add(mult(2, sum_choose), choose(precalc, j + i - 1, j));
    }
